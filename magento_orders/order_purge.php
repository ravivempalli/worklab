...
/* (c) Rrap Software Pvt Ltd.
   ...
   Usage :
   php export_purge.php [-t <to_time>] [-f <from_time>] [-o o1,o2...] -d <dir>
   More filters will be added later on.
   Creates the orders, items, etc in individual csv files in the named directory 
...
function usage()
{
  echo "Usage : php " . $argv[0] . "  [-t <to_time>] [-f <from_time>] [-o o1,o2...] -d <dir>\n";
  echo "Note : order list should not have spaces\n";
  exit;
}
...
for($i = 1; $i < count($argv); $i++){
 if($argv[$i] == '-f'){
   ...
   $lesserValue = $argv[$i + 1];
   $i++;
 }
 else{
   echo "unkown option " . $argv[$i] . "\n";
   usage();
 }
}

if($directory == NULL) usage();
if($greaterValue && $lesser_value) usage();
if($greaterValue && $order_list) usage();
if($lesserValue && $order_list) usage();
...
$order_csv = $directory. "/" . "order.csv";
$order_item_csv = $directory. "/" . "order_item.csv";
$quote_csv = $directory. "/" . "quote.csv";
$quote_item_csv = $directory. "/" . "quote_item.csv";
$oa_csv = $directory. "/" . "oa.csv";
$op_csv = $directory. "/" . "op.csv";
$osh_csv = $directory. "/" . "osh.csv";
$shipment_csv = $directory. "/" . "shipment.csv";
$shipment_item_csv = $directory. "/" . "shipment_item.csv";
$shipment_comment_csv = $directory. "/" . "shipment_comment.csv";
$shipment_track_csv = $directory. "/" . "shipment_track.csv";
$invoice_csv = $directory. "/" . "invoice.csv";
$invoice_item_csv = $directory. "/" . "invoice_item.csv";
$invoice_comment_csv = $directory. "/" . "invoice_comment.csv";
...
if($greaterValue){
              $orderCollection = $orderCollection->addFieldToFilter('created_at', array("gt" => $greaterValue))
              ->addFieldToFilter('updated_at', array("gt" => $greaterValue));
}else if($lesserValue){
              $orderCollection = $orderCollection->addFieldToFilter('created_at', array("lt" => $lesserValue))
              ->addFieldToFilter('updated_at', array("lt" => $lesserValue));
}else{
              $orderCollection = $orderCollection->addFieldToFilter('increment_id', array("in" => explode(",",$order_list)));
}
...
if($greaterValue){
              $invoiceCollection = $invoiceCollection->addFieldToFilter('created_at', array("gt" => $greaterValue))
              ->addFieldToFilter('updated_at', array("gt" => $greaterValue));
}else if($lesserValue){
              $invoiceCollection = $invoiceCollection->addFieldToFilter('created_at', array("lt" => $lesserValue))
              ->addFieldToFilter('updated_at', array("lt" => $lesserValue));
}else{
              $invoiceCollection = $invoiceCollection->addFieldToFilter('increment_id', array("in" => explode(",",$order_list)));
}
...
if($greaterValue){
              $shipmentCollection = $shipmentCollection->addFieldToFilter('created_at', array("gt" => $greaterValue))
              ->addFieldToFilter('updated_at', array("gt" => $greaterValue));
}else if($lesserValue){
              $shipmentCollection = $shipmentCollection->addFieldToFilter('created_at', array("lt" => $lesserValue))
              ->addFieldToFilter('updated_at', array("lt" => $lesserValue));
}else{
              $shipmentCollection = $shipmentCollection->addFieldToFilter('increment_id', array("in" => explode(",",$order_list)));
}
